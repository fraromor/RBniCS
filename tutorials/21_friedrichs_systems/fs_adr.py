...
class AdvectionDiffusionReaction(FriedrichsSystemProblem):
    def __init__(self, L, **kwargs):
        ...
        self.D_0 = self.A_1_0 * self.n[0] + self.A_2_0 * self.n[1]
        self.D_1 = self.A_1_1 * self.n[0]
        self.D_2 = self.A_2_1 * self.n[1]
    ...
    def compute_theta(self, term):
        mu = self.mu
        if term == "a":
            theta_a0kappa = 1/mu[0]  #kappa=viscosity
            theta_a0nu = mu[1]  #nu
            theta_a10 = 1.
            ...
            theta_D_0 = 1.
            theta_D_1 = mu[2]  #beta1
            theta_D_2 = mu[3]  #beta2
            theta_S = 1.
            theta_DpM_0 = 1.
            theta_DpM_1 = mu[2]  #beta1
            theta_DpM_2 = mu[3]  #beta2
            return (theta_a0kappa, theta_a0nu, theta_a10, theta_a11, theta_a20,
                    theta_a21, theta_D_0, theta_D_1, theta_D_2, theta_S,
                    theta_DpM_0, theta_DpM_1, theta_DpM_2)
        elif term == "f":
            theta_f0 = 1.
            return (theta_f0, )
        else:
    ...
    def assemble_operator(self, term):
        ...
        if term == "a":
            ...
            aD_0 = inner(dot(self.D_0('+'), avg(z)), jump(y)) * dS
            aD_1 = inner(dot(self.D_1('+'), avg(z)), jump(y)) * dS
            aD_2 = inner(dot(self.D_2('+'), avg(z)), jump(y)) * dS
            aS = self.alpha('+') * inner(jump(z), jump(y)) * dS
            aDpM_0 = 0.5 * inner(dot(self.D_0 + self.M, z), y) * ds
            aDpM_1 = 0.5 * inner(dot(self.D_1, z), y) * ds
            aDpM_2 = 0.5 * inner(dot(self.D_2, z), y) * ds
            return (a_a0kappa, a_a0nu, a_a10, a_a11, a_a20, a_a21, aD_0, aD_1,
                    aD_2, aS, aDpM_0, aDpM_1, aDpM_2)
        elif term == "f":
            f = self.f
            f0 = dot(y, f) * dx
            return (f0, )
...
problem = AdvectionDiffusionReaction(L,
                                     mesh=mesh,
                                     subdomains=subdomains,
                                     boundaries=boundaries)
mu_range = [(0.1, 1), (1, 1), (-1, 1), (1, 1)]
...
# Plot solution
snapshot = reduction_method.truth_problem.solve()
u_sol = snapshot.sub(2)
p = plot(u_sol, interactive=True)
plt.colorbar(p)
plt.show()
